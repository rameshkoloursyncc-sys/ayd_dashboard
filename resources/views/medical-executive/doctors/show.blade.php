...
<div class="px-4 pt-6">
    ...
    <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800 mb-4">
        <div class="grid grid-cols-6 gap-6">
            ...
            <div class="col-span-6 sm:col-span-3">
                ...
                <p class="text-gray-900 dark:text-white">{{ $doctor->pharmaCompany->name ?? 'N/A' }}</p>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        btn.addEventListener('click', function() {
            ...
            if (!doctorId) {
                console.error('No doctorId found on load-qr-btn (medical-executive)');
                return;
            }

            console.log('Fetching QR for doctorId (medical-executive):', doctorId);
            ...
            fetch('https://ayd.pinktreehealth.com/api/qr/' + doctorId)
                .then(function(res) {
                    console.log('QR API raw response status (medical-executive):', res.status);
                    if (!res.ok) throw new Error('API request failed');
                    return res.json();
                })
                    .then(function(json) {
                        console.log('QR API parsed JSON (medical-executive):', json);
                        if (json.status && json.data && json.data.qrCode) {
                            console.log('QR data payload (medical-executive):', json.data.qrData || {});
                        ...
                        console.warn('QR API returned unsuccessful status or missing data (medical-executive):', json);
                        qrContainer.innerHTML = '<p class="text-xs text-red-500">Failed to load QR code.</p>';
                    }
                })
                .catch(function(err) {
                    console.error('QR fetch error (medical-executive):', err);
