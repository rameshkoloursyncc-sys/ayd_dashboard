                    ...
                    @forelse($doctors as $doctor)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                @if(empty($doctor->pharmaCompanyName) || $doctor->pharmaCompanyName == 'N/A')
                                    ...
                                    <form action="{{ route('superadmin.doctors.attachPharma', $doctor->api_id) }}" method="POST" class="inline-flex items-center mt-2">
                                        ...
                                        <select name="pharma_company_id" class="mr-2 rounded border-gray-300">
                                            ...
                                            @foreach((app(\App\Services\PinktreeApiService::class)->getAllPharma()->json('data') ?? []) as $pharma)
                                                <option value="{{ $pharma['_id'] }}">{{ $pharma['name'] }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <td class="p-4 space-x-2 whitespace-nowrap">
                                ...
                                @endif
                                {{-- Approve button for Super Admin (AJAX, only if not already Approved) --}}
                                @if(($doctor->approvalStatus ?? '') !== 'Approved')
                                    <button type="button" class="approve-btn inline-flex items-center p-2 text-sm font-medium text-center text-white rounded-lg bg-emerald-600 hover:bg-emerald-700 focus:ring-4 focus:ring-emerald-300 dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800" data-api-id="{{ $doctor->api_id }}">
                                        Approve
                                    </button>
                                @endif
                                {{-- Modal pushed after the doctors loop to avoid duplicates --}}
                            </td>
                        </tr>
                    @empty
                        <tr>
