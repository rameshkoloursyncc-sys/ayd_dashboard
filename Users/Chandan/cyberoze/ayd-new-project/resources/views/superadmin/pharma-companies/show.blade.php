...
<div class="px-4 pt-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-primary-700 dark:text-primary-300 flex items-center gap-2">
            ...
            {{ $pharmaCompany['name'] ?? 'Pharma Company' }}
        </h1>
        <div class="flex items-center gap-2">
            @if(optional(auth()->user())->role === 'super_admin')
                <a href="{{ route('superadmin.pharma-companies.edit', $pharmaCompany['_id']) }}" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2.5 dark:bg-blue-500 dark:hover:bg-blue-600 focus:outline-none dark:focus:ring-blue-800">Edit Company</a>
            @endif
            <a href="{{ route('superadmin.pharma-companies.index') }}" class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">Back to list</a>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700 flex flex-col items-center">
            <div class="w-full">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <svg class="w-5 h-5 text-primary-600 dark:text-primary-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 15c2.5 0 4.847.655 6.879 1.804" /></svg>
                    Admin User
                </h2>
                @if(isset($adminUser) && $adminUser)
                    <div class="mb-2">
                        <span class="block text-sm font-medium text-gray-700 dark:text-gray-300">Name:</span>
                        <span class="block text-base text-gray-900 dark:text-white">{{ $adminUser->name }}</span>
                    </div>
                    <div>
                        <span class="block text-sm font-medium text-gray-700 dark:text-gray-300">Email:</span>
                        <span class="block text-base text-gray-900 dark:text-white">{{ $adminUser->email }}</span>
                    </div>
                @else
                    <p class="text-gray-500 dark:text-gray-400">No admin user found for this pharma company.</p>
                @endif
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700 flex flex-col items-center">
